...
type result_internal =
  ...
  | Camlot_Optimal
  | Camlot_Unbounded
  | Camlot_MaxIterReached

type result =
  | Infeasible
  | Unbounded
  | Optimal of { cost : float; coupling : mat; u : vec; v : vec }
  | MaxIterReached of { cost : float; coupling : mat; u : vec; v : vec }
...
external kanto_solve : vec -> vec -> mat -> mat -> vec -> vec -> fref -> int -> result_internal = "camlot_stub_bytecode" "camlot_stub_native"
...
let kantorovich ~x ~y ~d ~num_iter =
  ...
  | Camlot_Unbounded ->
    Unbounded
  | Camlot_Optimal ->
    Optimal { cost = cost.field
            ; coupling = gamma
            ; u
            ; v
            }
  | Camlot_MaxIterReached ->
    MaxIterReached { cost = cost.field
                   ; coupling = gamma
                   ; u
                   ; v
                   }
